...
-- Câu lệnh chính để xuất dữ liệu cho mẫu biểu
SELECT DISTINCT -- Sử dụng DISTINCT để loại bỏ các dòng trùng lặp do join với bảng ChiTietBaiThi
    -- Thông tin chung và thông tin sinh viên
    GETDATE() AS NgayLap, -- Hoặc NOW() nếu dùng MySQL. Lấy ngày giờ hiện tại
    sv.MaSV AS Ma,
    sv.HoTenSV,
    sv.NgaySinh,
    sv.DiaChi,
    ...
    -- Thông tin trong bảng danh sách bài thi
    ROW_NUMBER() OVER(PARTITION BY sv.MaSV ORDER BY bt.NgayThi, bt.MaBT) AS STT,
    bt.TenBaiThi,
    bt.NgayThi,
    bt.TGLamBai AS ThoiGianLamBai -- Lấy dữ liệu từ cột TGLamBai
FROM
    SinhVien AS sv
JOIN
    ChiTietBaiThi AS ctbt ON sv.MaSV = ctbt.MaSV -- Kết nối SinhVien với ChiTietBaiThi
JOIN
    BaiThi AS bt ON ctbt.MaBT = bt.MaBT          -- Kết nối ChiTietBaiThi với BaiThi
WHERE
    sv.MaSV = @MaSinhVien; -- Lọc kết quả theo mã sinh viên chỉ định
